...
void	ClapTrap::attack(const std::string& target){
	...
	if (_EnergyPoints <= 0){
		std::cout << _name << " has " << _EnergyPoints << "energy points making it unable to attack" << std::endl;
		return ;
	}
	--this->_EnergyPoints;
...
void	ClapTrap::beRepaired(unsigned int amount){
	...
	if (this->_EnergyPoints <= 0){
		std::cout << _name << "has " << _EnergyPoints 
		<< "energy points making it unable to attack" << std::endl;
		return ;
	}
	--this->_EnergyPoints;
	std::cout << _name <<" Heal " << amount << "Hp!" << std::endl;
}